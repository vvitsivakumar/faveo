...
foreach ($depts as $dept) {
	...
	foreach ($overdues as $overdue) {
      ...
      $sla_plan = App\Model\helpdesk\Manage\Sla\Sla_plan::where('id','=',$overdue->sla)->first();
		$ovdate = $overdue->created_at;
		// $new_date = date_add($ovdate, date_interval_create_from_date_string($sla_plan->grace_period)).'<br/><br/>';
    $new_date = date_add($ovdate, date_interval_create_from_date_string($overdue->duedate)).'<br/><br/>';
        ...
        <tr>
            <td style="padding: " colspan="2" bgcolor="#FFFFFF">
                ...
                <p style="padding: 0px 30px 20px 50px; margin: 0; font-size: 16px; color: #333; font-weight: normal; font-family: Arial, Helvetica, sans-serif; line-height: 22px; text-align: center; color: #3AB4FF;">
                 ...
                 <strong>{!! Lang::get('lang.powered_by') !!}<a href="https://www.productdemourl.com/" target="_blank">Helpdesk</a></strong>
                 @else
                 <strong>{!! Lang::get('lang.powered_by') !!} <a href="http://www.faveohelpdesk.com" target="_blank">Faveo</a></strong>
                 @endif
