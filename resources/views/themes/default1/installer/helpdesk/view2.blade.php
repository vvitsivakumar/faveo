...
function validate_php(&$results) {
    if (version_compare(PHP_VERSION, '7.1') == -1) {
        $results[] = new TestResult('Minimum PHP version required in order to run Faveo HELPDESK is PHP 7.1.3 or greater Your PHP version: ' . PHP_VERSION, STATUS_ERROR);
        return false;
    } else {
...
function validate_extensions(&$results) {
    ...
    $required_extensions = ['openssl', 'tokenizer', 'pdo', 'mysqli', 'fileinfo', 'curl', 'zip', 'mbstring', 'xml'];
