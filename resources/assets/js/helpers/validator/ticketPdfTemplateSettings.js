 ...
 * */
export function validateTicketPdfTemplateSettings(data){

    const { template, name, maximum_logs } = data;

    //rules has to apply only after checking conditions
    let validatingData = {
        template: [template, 'isRequired'],
        name: [name, 'isRequired'],
        maximum_logs : [maximum_logs, 'isRequired', { 'minValue(10)' : lang('max_log_validation')}]
    };
