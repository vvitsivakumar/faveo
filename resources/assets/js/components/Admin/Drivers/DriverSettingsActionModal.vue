...
export default {
  name: "DriverSettingsActivateModal",

  description: "drivers table action modal component",
  ...
  methods:{
    ...
    onSubmit(){
      ...
      axios.post('api/post/search-driver',{
        'driver' : this.driver.key,
        'status' : 1
      }).then(res=>{

        window.emitter.emit('DriversIndexrefreshData');  // Emit an event for data refresh
