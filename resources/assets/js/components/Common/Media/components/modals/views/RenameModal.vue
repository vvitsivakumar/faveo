<template>
	<div class="modal-content fm-modal-rename">
		<div class="modal-header">
			<h5 class="modal-title">{{ lang1.modal.rename.title }}</h5>
		...
		<div class="modal-body">
			<div class="form-group">
				<label for="fm-input-rename">{{ lang1.modal.rename.fieldName }}</label>
				...
				<div class="invalid-feedback" v-show="checkName">
					{{ lang1.modal.rename.fieldFeedback }}
					{{ directoryExist ? ` - ${lang1.modal.rename.directoryExist}` : ''}}
					{{ fileExist ? ` - ${lang1.modal.rename.fileExist}` : ''}}
				</div>
			</div>
		</div>
		<div class="modal-footer">
			<button class="btn btn-info"
					...
					v-on:click="rename">{{ lang1.btn.submit }}
			</button>
