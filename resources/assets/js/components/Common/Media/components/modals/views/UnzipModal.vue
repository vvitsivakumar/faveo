<template>
	<div class="modal-content fm-modal-unzip">
		<div class="modal-header">
			<h5 class="modal-title">{{ lang1.modal.unzip.title }}</h5>
		...
		<div class="modal-body">
			<div class="d-flex justify-content-between">
				<div>
					<strong>{{ lang1.modal.unzip.fieldRadioName }}</strong>
				</div>
				...
				<div class="form-check form-check-inline">
					...
					<label class="form-check-label" for="unzipRadio2">
						{{ lang1.modal.unzip.fieldRadio2 }}
					</label>
				</div>
			</div>
			<hr>
			<div v-if="createFolder" class="form-group">
				<label for="fm-folder-name">{{ lang1.modal.unzip.fieldName }}</label>
				...
				<div class="invalid-feedback" v-show="directoryExist">
					{{ lang1.modal.unzip.fieldFeedback }}
				</div>
			</div>
			<span v-else class="text-danger">{{ lang1.modal.unzip.warning }}</span>
		</div>
		<div class="modal-footer">
			<button class="btn btn-info"
					v-bind:disabled="!submitActive"
					v-on:click="unpackArchive">{{ lang1.btn.submit }}
			</button>
			<button class="btn btn-light" v-on:click="hideModal">{{ lang1.btn.cancel }}</button>
		</div>
	</div>
</template>
