...
<div v-for="(item, index) in alertContent" :key="index" >
  ...
  <div ref="alertWrapperElements" v-if="item.use === 1" class="alert-warning alert-wrapper mb-0 pt-2 pl-3 pb-1"
    ...
    <button type="button" @click="onCloseAlert(item)" class="close pt-2 pl-4 pr-3" id="alert_close">×</button>
...
<!--  </div>-->
	<div class="wrapper" v-bind:style="{ top: alertContent ? this.alertWrapper +'px !important' : '' }">
		...
		<agent-nav-bar v-bind:style="{ top: alertContent ? this.alertWrapper +'px !important' : '' }"  :auth="auth" :portal="portal"></agent-nav-bar>

		<agent-side-bar  v-bind:style="{ top: alertContent ? this.alertWrapper +'px !important' : '' }" :alertWrapper="alertWrapper" :alertData="alertContent" :auth="auth" :company="company" :portal="portal"></agent-side-bar>
				...
				<div class="container-fluid">
					...
					<div  v-if="dummyInstall" class="alert alert-info alert-dismissible">
						...
						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
							...
							<template v-if="auth.user_data.role == 'admin'">
								...
								{{trans('dummy_data_installation_message')}}&nbsp;<a :href="basePath()+'/admin/clean-dummy-data'">{{trans('click')}}&nbsp;</a>{{trans('clear-dummy-data')}}
							</template>
					...
					<div v-if="!dummyInstall && !isMailConfigured" class="alert alert-warning">&nbsp;
					  ...
					  <template v-if="auth.user_data.role == 'admin'">
							...
							<a :href="basePath()+'/admin/email/create'">{{trans('confihure-the-mail-now')}}</a>
					  </template>
...
<!--							<router-view :key="$route.fullPath" :auth="auth" :isFAEnforced="isFAEnforced"></router-view>-->
<!--						</transition>-->
						...
						<router-view v-slot="{ Component, route }">
							...
							<transition name="fade">
								...
								<component :is="Component" :key="route.fullPath" :auth="auth" :isFAEnforced="isFAEnforced"/>
							</transition>
						</router-view>
...
<script>
	...
	import AgentNavbar from "./LayoutComponents/AgentNavbar.vue";
	import AgentSidebar from "./LayoutComponents/AgentSidebar.vue";
	import AgentBreadcrumbs from "./LayoutComponents/AgentBreadcrumbs.vue";
	import AgentFooter from "./LayoutComponents/AgentFooter.vue";
	import HelpSection from "../HelpSection/HelpSection.vue";
	...
	export default {
		...
		props : {
			...
			isFAEnforced : { type : [Boolean, Number], default : false },
      ...
      isUpdatedVersion : { type : [Boolean, Number], default : false }
		...
		components : {
			...
			'agent-nav-bar' : AgentNavbar,

			'agent-side-bar' : AgentSidebar,

			'agent-bread-crumbs' : AgentBreadcrumbs,

			'agent-footer' : AgentFooter,

			'help-widget' : HelpSection
		},
